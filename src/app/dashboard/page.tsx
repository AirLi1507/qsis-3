...
const QuickNav = (prop: QuickNavProp) => {
  return (
    <Link href={prop.url}>
      <div className="text-black/25 hover:text-black/75 dark:text-white/25 dark:hover:text-white/75 h-64 md:h-44 bg-white/25 dark:bg-black/25 rounded-xl dark:border-2 hover:border-black/75 dark:border-white/25 dark:hover:border-white/75 shadow-lg p-4 pt-6 box-border duration-250">
...
const Dashboard = () => {
  ...
  return (
    <div className="w-full h-full m-2 flex flex-col">
      <span className="text-2xl font-medium w-full bg-white/45 dark:bg-white/20 rounded-lg shadow-md p-6 duration-750 select-none">{t("quick_nav")}</span>
      <div className="w-full h-fit mt-4 grid grid-cols-2 gap-4 p-2 pb-6 md:pb-0 overflow-y-scroll">
